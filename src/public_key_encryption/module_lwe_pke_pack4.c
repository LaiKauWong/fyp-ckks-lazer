...
/*
 * Experimental variant: 4 bits per coefficient
 *
 * Mapping:
 *   symbol in [0, 15]  ->  round(symbol * q / 16)
 *
 * Intended as a separate experimental file.
 * Keep the original implementation unchanged.
 */

#define LWE_PKE_BITS_PER_COEFF 4
...
int lwe_pke_encode_bits_poly(const lwe_pke_ctx_t ctx, poly_t m,
  ...
  for (size_t coeff_idx = 0; coeff_idx < required_coeffs; coeff_idx++) {
    ...
    /* target ≈ symbol * q / 16 */
    int64_t coeff = ((int64_t)symbol * q + (LWE_PKE_LEVELS / 2)) / LWE_PKE_LEVELS;
...
/* 4-bit packing decode: nearest of 16 levels */
int lwe_pke_decode_poly_bits(const lwe_pke_ctx_t ctx, uint8_t *msg_out,
  ...
  for (size_t coeff_idx = 0; coeff_idx < required_coeffs; coeff_idx++) {
    ...
    for (uint8_t symbol = 0; symbol < LWE_PKE_LEVELS; symbol++) {
      int64_t target = ((int64_t)symbol * q + (LWE_PKE_LEVELS / 2)) / LWE_PKE_LEVELS;
      int64_t d = xm - target;
      d %= q;
      if (d < 0) d += q;
      if (d > q - d) d = q - d;
